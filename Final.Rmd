...
```{r}
res <- resamples(list(LDA = model.lda, NB = model.nb, Rpart = rpart.fit, ctree = ctree.fit, glm = model.glm, glmn = model.glmn, Mars = model.mars))
summary(res)
...
auc <- c(roc.lda$auc[1], roc.nb$auc[1],roc.rpart$auc[1], roc.ctree$auc[1])
...
modelNames <- c("lda","nb", "rpart", "ctree", "glm", "glmn", "mars")
